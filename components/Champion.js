...
export default class extends React.Component {
    ...
    render() {
      const spells = this.props.data.spells.map((spell) => ( <Spell key={spell.name} isAdvanced={this.props.isAdvanced} data={spell} />))
      const stats = this.props.data.stats
      const statHeaders = Object.keys(stats).map((key) => (<tr><th>{key}</th><td>{stats[key]}</td></tr>))
      return(

        <div key={this.props.data.name} className="bg-white flex flex-col p-1 border-solid border-2 border-gray-600 leading-tight">
